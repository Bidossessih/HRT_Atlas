...
con <- dbConnect(RSQLite::SQLite(), "/mnt/d/Repository_ubuntu/Housekeeping/HRT_Atlas_db/LogMCF_Housekeeping_human_mouse.sqlite")

con1 <- dbConnect(RSQLite::SQLite(), "/mnt/d/Repository_ubuntu/Housekeeping/HRT_Atlas_db/MCF_Housekeeping_Mouse_mouse.sqlite")
...
load("www/Housekeeping_TranscriptsMouse.RData")#sqlite ==> down
#load("External/Data/DesignedPrimer/sampleTable.RData")
load("External/Data/DesignedPrimer/RefInfoHumanHK2.RData") #Not included in sqlite. Code will change
load("External/Data/primer/RefInfoMouseHK2.RData") #Not included in sqlite. Code will change
#Load HK_RPKM  #sqlite ==> down
...
#Merge HK_RPKM #sqlite ==> down

HK_RPKM = cbind(HK_RPKM1,HK_RPKM2,HK_RPKM3,HK_RPKM4)
#Remove after merging #sqlite ==> down
rm(HK_RPKM1,HK_RPKM2,HK_RPKM3,HK_RPKM4)

load("External/Data/Housekeeping_Genes.RData") # to be included see new project inst/app/www/data
...
load("External/Data/Housekeeping_TranscriptFiltered.RData")#sqlite ==> down
...
####
GeneInfo = GeneInfo[,c(1,2,5,3,4,6,7)]
GeneInfo = GeneInfo[,1:6]
geneMouse = geneMouse[,c(1, 2, 5, 4, 3, 6)]

colnames(GeneInfo) = colnames(geneMouse) = c("Symbol",
                                             "entrezgene",
                                             "Name",
                                             "Summary",
                                             "Synonym" ,
...
full_info = rbind(data.frame(GeneInfo, "specie"="Human"),
                  data.frame(geneMouse, "specie"="Mouse"))

colnames(Mouse_HK_genes)[1:2] = colnames(HK_geneAtualizadoRef)[c(2,1)]

ens_info = rbind(HK_geneAtualizadoRef[,1:2],
                 Mouse_HK_genes[,c(2,1)])

colnames(ens_info)[2]="Symbol"

full_info = merge(ens_info, full_info, by="Symbol", all.y = TRUE)


save(full_info, file = "External/Data/Full_HK_Human_Mouse_Info.RData")
